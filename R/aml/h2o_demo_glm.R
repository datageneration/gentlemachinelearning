...
# Read data into R (not H2O)
airlinedata = read.csv("https://s3.amazonaws.com/h2o-airlines-unpacked/allyears2k.csv")
library(descr)
freq(airlinedata$IsDepDelayed)
...
originFlights = h2o.group_by(data = airlines.hex, by =
"Origin", nrow("Origin"),gb.control=list(na.methods="rm"))
originFlights.R = as.data.frame(originFlights)
# Find number of flights per month
flightsByMonth = h2o.group_by(data = airlines.hex, by
                              = "Month", nrow("Month"),gb.control=list(na.methods="rm"))
...
summary(pred$predict, exact_quantiles = TRUE)
